...
- **`BaseController::json()`**: Added `ob_end_clean()` loop before setting headers to clear any PHP warnings/notices that may have been buffered, preventing them from corrupting the JSON output.
...
## [1.7.87] - 2026-05-20
### Fixed
- **Όψη Έργου**: Το dropdown "Αλλαγή Κατάστασης" έκρβε επιλογές όταν το mouse ήταν πάνω από το card — αφαιρέθηκε το `transform: translateY(-2px)` που δημιουργούσε νέο CSS stacking context

## [1.7.86] - 2026-05-20
### Fixed
- **Όψη Έργου**: Το dropdown "Αλλαγή Κατάστασης" εμφανίζεται πίσω από άλλα στοιχεία (z-index fix)

## [1.7.85] - 2026-05-20
### Fixed
- **Εργασίες Ημέρας PDF**: Διόρθωση εμφάνισης λογότυπου στο header (λανθασμένο path separator)
- **Εργασίες Ημέρας PDF**: Footer εμφανίζει πλέον όλα τα στοιχεία εταιρείας (επωνυμία, διεύθυνση, ΑΦΜ, τηλέφωνο, email, website)
...
- **Συντηρήσεις Μ/Σ**: Προσθήκη ελέγχου όταν το PHP αδειάζει σιωπηλά τα POST data λόγω υπέρβασης ορίου — εμφανίζεται πλέον κατανοητό μήνυμα σφάλματος αντί σιωπηλής αποτυχίας

## [1.7.82] - 2026-05-15
### Fixed
- **Συντηρήσεις Μ/Σ**: Print layout — τίτλος εμφανίζεται πριν από τον πίνακα (όχι από κάτω)
- **Συντηρήσεις Μ/Σ**: Print layout — A4 landscape, πλήρες πλάτος χωρίς κοπή
- **Συντηρήσεις Μ/Σ**: Print layout — κρύβει footer και sidebar κατά την εκτύπωση

## [1.7.81] - 2026-05-15
### Added
- **Συντηρήσεις Μ/Σ**: Κουμπί "Εκτύπωση" δίπλα στα quick-filter buttons — εκτυπώνει την τρέχουσα φιλτραρισμένη λίστα
- **Συντηρήσεις Μ/Σ**: Print layout: κρύβει sidebar/φίλτρα/κουμπιά, εμφανίζει τίτλο με το ενεργό φίλτρο και ημερομηνία εκτύπωσης

## [1.7.80] - 2026-05-15
### Added
- **Συντηρήσεις Μ/Σ**: Quick-filter buttons για Επόμενες Συντηρήσεις — Ληξιπρόθεσμες, 1 Μήνας, 3 Μήνες, 6 Μήνες
- **Συντηρήσεις Μ/Σ**: Ταξινόμηση κατά `next_maintenance_date ASC` όταν ενεργό upcoming filter (πιο επείγουσες πρώτα)
- **Dashboard**: Αυτόματο banner ειδοποίησης για ληξιπρόθεσμες και επερχόμενες συντηρήσεις (εμφανίζεται μόνο όταν υπάρχουν)

## [1.7.62] - 2026-05-12
### Fixed
- **Προσφορά Συντήρησης**: Word και PDF export μέσω LibreOffice headless — ακριβής αναπαραγωγή του .docx template

## [1.7.61] - 2026-05-12
### Fixed
- **Προσφορά Συντήρησης**: PDF export ανασχεδιάστηκε βάσει πραγματικού template — logo, φωτ. μετασχηματιστή, ISO badge, πλήρης περιγραφή εργασιών, κοστολόγηση, υπογραφή
- **Προσφορά Συντήρησης**: Αυτόματη εξαγωγή εικόνων από .docx template κατά το πρώτο PDF render
...
- **Προσφορά Συντήρησης**: Λίστα προσφορών με radio button αποδοχής (timestamp) + inline ημερομηνία προγραμματισμού
- **Προσφορά Συντήρησης**: Export σε PDF (TCPDF) με πλήρες επαγγελματικό layout
- **Προσφορά Συντήρησης**: Export σε Word (.docx) μέσω template με placeholders XXX/XXXX/XXXXX
- **Προσφορά Συντήρησης**: Αποστολή email με συνημμένο PDF
- **Προσφορά Συντήρησης**: Αυτόματος υπολογισμός τιμής (1Μ/Σ=400€, 2=600€, 3=900€, 4+=1.200€) με δυνατότητα custom τιμής
- **Menu**: Νέο στοιχείο "Προσφορά Συντήρησης" αριστερά, κάτω από Συντηρήσεις Υ/Σ

## [1.7.58] - 2026-05-11
### Fixed
- **Update notifications**: Σύγκριση `installed_version` με `latest_version`, ώστε να μην εμφανίζεται ψευδής ειδοποίηση νέας έκδοσης.
- **Εργασίες Ημέρας**: Το summary ωρών αθροίζει πλέον τις ώρες τεχνικών.
- **Αναφορές**: Συμπερίληψη εσόδων Συντηρήσεων Υ/Σ στο ενοποιημένο revenue.
- **Μεταφράσεις**: Διόρθωση `invoices.change_status` και συμπλήρωση κενών ελληνικών i18n keys.

### Changed
- **Υλικά**: Τυποποίηση μονάδων μέτρησης με canonical dropdown και migration υπαρχόντων εγγραφών.

## [1.7.57] - 2026-05-08
### Fixed
- **Αναφορές**: Συμπερίληψη εσόδων Συντηρήσεων Υ/Σ στο συνολικό revenue (getRevenueData + getSummaryStats)
- **Αναφορές**: Ενοποιημένα έσοδα = Έργα + Συντηρήσεις για ακριβή οικονομική ανάλυση

## [1.7.56] - 2026-05-08

### 🔧 Improvements
- **Υλικά - Μονάδες**: Τυποποίηση μονάδων μέτρησης — dropdown με 18 κανονικές τιμές αντί ελεύθερου κειμένου σε εργασίες & ημερήσιες εργασίες
- **Υλικά - Migration**: `normalize_material_units.sql` για εξυγίανση υπαρχόντων εγγραφών στις `materials_catalog`, `task_materials`, `materials`
- **UpdateChecker**: Ανάγνωση έκδοσης από `VERSION` αρχείο αντί σταθεράς `APP_VERSION` — διόρθωση ψευδούς ειδοποίησης "νέα έκδοση διαθέσιμη"
- **config.php**: Συγχρονισμός `APP_VERSION` στην τρέχουσα έκδοση
...
### 🔄 Changes
- **Menu**: Αλλαγή ετικέτας αριστερού μενού "Πληρωμές" σε "Μισθοδοσία"
...
### 🐛 Bug Fixes
- **Login Page**: Διόρθωση εμφάνισης σελίδας σύνδεσης σε desktop — αφαίρεση `height: 100vh` / `overflow: hidden` που έκοβε το περιεχόμενο σε μικρότερες αναλύσεις
...
### 🐛 Bug Fixes
- **Εργασίες Έργου**: Τα κουμπιά "Μεταφορά" και "Διαγραφή" δεν λειτουργούσαν για εργασίες με αλλαγές γραμμής στην περιγραφή (multiline description) — το literal newline μέσα στο `onclick` attribute προκαλούσε JavaScript syntax error. Πλέον αφαιρούνται τα newlines πριν την κωδικοποίηση.
- **Προβολή Εργασίας (view)**: Προστέθηκε κουμπί "Διαγραφή" που έλειπε εντελώς από τη σελίδα προβολής εργασίας.
...
### 🐛 Bug Fixes
- **PDF**: ΣΥΓΚΕΝΤΡΩΤΙΚΑ ΣΤΟΙΧΕΙΑ δεν σπάνε πλέον σε δύο σελίδες — μεταφέρονται ολόκληρα στην επόμενη σελίδα με `nobr`
...
### 🐛 Bug Fixes
- **PDF Συγκεντρωτικά Ημερομίσθια**: Αφαιρέθηκε η στήλη ΗΜΕΡΟΜΙΣΘΙΑ από τον πίνακα ανά τεχνικό — απέφευγε σύγχυση με το συνολικό (365 ανά τεχνικό ≠ 361 σύνολο)
- Το σύνολο ημερομίσθιων (ΣΩΡΟ ωρών / 8, μία στρογγυλοποίηση) εμφανίζεται μόνο στα ΣΥΓΚΕΝΤΡΩΤΙΚΑ ΣΤΟΙΧΕΙΑ
...
### 🔄 Changed
- **Ημερομίσθια**: Αλλαγή μεθόδου σε `CEIL(Σύνολο Ωρών / 8)` με μία μόνο στρογγυλοποίηση σε όλα τα σημεία (tab badge, footer, PDF, στατιστικά)
...
### 🐛 Bug Fixes
- **Ημερομίσθια Υπολογισμός**: Διορθώθηκε σε όλα τα σημεία — χρησιμοποιείται `CEIL(SUM(ωρών τεχνικού) / 8)` ανά τεχνικό
- **Tab Badge**: Εμφανίζει σωστό σύνολο ημερομίσθιων ανά τεχνικό
- **Footer Tab**: Εμφανίζει ανάλυση ανά τεχνικό (ώρες + ημερομίσθια) + σύνολο
- **PDF Αναφορά**: Διορθώθηκε σε `CEIL(SUM/8)` ανά τεχνικό αντί για `SUM(CEIL/8)` ανά εγγραφή
- **Στατιστικά Tab**: Προστέθηκε στήλη ημερομίσθια ανά τεχνικό, κάρτα ημερομίσθιων αντί ημερών εργασίας
...
### 🔄 Changes
- **Συμφωνητικό κόστος**: Merged separate "Αξία Υλικών" and "Αξία Εργασίας" rows into a single "Κόστος υλικών και εργασίας = X €" line in the contract cost table.
...
### 🔴 Critical Fixes
- **UpdateController**: Fixed fatal error - now extends `BaseController` instead of `BaseModel`, added proper authentication and admin checks
- **TrashController**: Fixed fatal error - refactored to extend `BaseController`, added CSRF validation to all POST methods
- **Settings Model**: Fixed completely broken model - rewrote from MySQLi to PDO (was using `bind_param()` on PDO connection)
- **Material Model**: Fixed fatal `Database::getInstance()` calls - replaced with proper `new Database()` pattern
- **TechniciansController**: Fixed missing methods - replaced `checkCSRF()` with `validateCsrfToken()`, replaced `checkPermission()` with `can()`

### 🟡 Security Fixes
- **XSS Vulnerability**: Fixed 38+ unescaped `$_SESSION['success']`/`$_SESSION['error']` outputs across 26 view files - wrapped all with `htmlspecialchars()`
- **CSRF Protection**: Added missing `<meta name="csrf-token">` tag to header.php for AJAX requests
- **TrashController**: Added CSRF validation to 5 POST methods (restore, permanentDelete, bulkRestore, bulkDelete, emptyTrash)

### 🟢 Performance & Code Quality
- **BaseController**: Added user data caching in `getCurrentUser()` - eliminates N+1 query problem on role checks
- **index.php**: Removed 60+ lines of dead Router code that was never dispatched
- **Customer Dropdown**: Fixed dropdown overlap issue on cards view using Bootstrap Popper strategy: fixed
- **Customer List View**: Added delete button with CSRF protection

### 🌍 Translations
- **Greek Translations**: Added missing `common.update` ("Ενημέρωση") and `common.create` ("Δημιουργία") keys

### 📝 Code Cleanup
- **Deleted Dead Files Locally**: Removed `config2.php`, `config-WORKING.php.backup`, `AuthController_OLD.php`, `AuthController_NEW_METHODS.txt`, `SettingsController.php.backup`, `EmailService_OLD.php`
- **GitHub Cleanup Needed**: Credential files (`config2.php`, `config-WORKING.php.backup`) still need manual deletion from GitHub repo

### ⚠️ Breaking Changes
None - all fixes are backward compatible

### 🔧 Technical Details
- All controller fixes ensure proper MVC patterns
- All database calls now use consistent PDO interface
- All POST routes now have CSRF protection
- All user-facing output now properly escaped
