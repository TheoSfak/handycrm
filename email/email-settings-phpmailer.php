...
$settings_row = $pdo->query("SELECT * FROM smtp_settings LIMIT 1")->fetch(PDO::FETCH_ASSOC);

// Convert to old format for compatibility with form
$settings = [];
if ($settings_row) {
    $settings['smtp_host'] = $settings_row['host'] ?? '';
    $settings['smtp_port'] = $settings_row['port'] ?? '465';
    $settings['smtp_username'] = $settings_row['username'] ?? '';
    $settings['smtp_password'] = $settings_row['password'] ?? '';
    $settings['smtp_encryption'] = $settings_row['encryption'] ?? 'ssl';
    $settings['from_email'] = $settings_row['from_email'] ?? '';
    $settings['from_name'] = $settings_row['from_name'] ?? '';
    $settings['reply_to'] = $settings_row['from_email'] ?? '';
}
