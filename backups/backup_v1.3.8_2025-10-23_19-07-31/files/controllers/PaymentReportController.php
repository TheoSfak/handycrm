...
class PaymentReportController extends BaseController {
    ...
    private function generatePDF($data, $dateFrom, $dateTo, $paidStatus, $grandTotalHours, $grandTotalAmount) {
        ...
        foreach ($data as $techData) {
            ...
            $html .= '<td style="width: 27%;">' . __('payments.task') . '</td>';
            $html .= '<td style="width: 8%; text-align: center;">' . __('payments.hours') . '</td>';
            $html .= '<td style="width: 13%; text-align: right;">' . __('payments.rate') . '</td>';
            ...
            foreach ($entries as $entry) {
                ...
                $html .= '<td style="width: 10%;">' . formatDate($workDate) . '</td>';
                $html .= '<td style="width: 20%;">' . htmlspecialchars($entry['project_title']) . '</td>';
                $html .= '<td style="width: 27%;">' . htmlspecialchars($entry['task_description']) . '</td>';
                $html .= '<td style="width: 8%; text-align: center;">' . number_format($entry['hours_worked'], 2) . '</td>';
                $html .= '<td style="width: 13%; text-align: right;">' . formatCurrency($entry['hourly_rate']) . '</td>';
                ...
                if ($isPaid) {
                    ...
                    $html .= '<td style="width: 12%; text-align: center; font-size: 8px; color: #0f5132;">' . $paidInfo . '</td>';
                } else {
                    $html .= '<td style="width: 12%; text-align: center; color: #999;">-</td>';
                }
