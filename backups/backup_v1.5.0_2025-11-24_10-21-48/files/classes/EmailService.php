...
class EmailService {
    ...
    private function loadSMTPSettings() {
        try {
            // Get table structure
            $stmt = $this->pdo->query("DESCRIBE smtp_settings");
            $columns = $stmt->fetchAll(PDO::FETCH_COLUMN);
            
            // Query all settings
            $stmt = $this->pdo->query("SELECT * FROM smtp_settings WHERE is_active = 1 LIMIT 1");
            ...
            if ($stmt) {
                ...
                if ($result && count($result) > 0) {
                    ...
                    // Map database columns to internal format
                    $this->smtp_settings = [
                        'smtp_host' => $row['host'] ?? $row['smtp_host'] ?? '',
                        'smtp_port' => $row['port'] ?? $row['smtp_port'] ?? 587,
                        'smtp_username' => $row['username'] ?? $row['smtp_username'] ?? '',
                        'smtp_password' => $row['password'] ?? $row['smtp_password'] ?? '',
                        'smtp_encryption' => $row['encryption'] ?? $row['smtp_encryption'] ?? 'tls',
                        'from_email' => $row['from_email'] ?? '',
                        'from_name' => $row['from_name'] ?? 'HandyCRM',
                        'reply_to' => $row['reply_to'] ?? $row['from_email'] ?? ''
                    ];
                    
                    error_log("SMTP Settings loaded successfully");
                } else {
                    error_log("No active SMTP settings found in database");
                    $this->smtp_settings = [];
                }
            }
